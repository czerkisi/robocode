...
public class MilwaukeeMan extends Robot {
    ...
    private List<EnemyRobot> enemyRobots;
    ...
    private void avoidRobots() {
        ...
        for (EnemyRobot enemy : enemyRobots) {
            List<Coordinate> coordinates = enemy.getCoordinates();
        ...
        if (closestDistance < safeDistance && closestEnemy != null) {
    ...
    public void onScannedRobot(ScannedRobotEvent event) {
        ...
        EnemyRobot enemy = findEnemyRobot(event.getName());

        if (enemy == null) {
            enemy = new EnemyRobot(event.getName());
            enemyRobots.add(enemy);
        }
    ...
    private void determineAction(EnemyRobot enemy) {
        List<Coordinate> coordinates = enemy.getCoordinates();
    ...
    private EnemyRobot findEnemyRobot(String name) {
        for (EnemyRobot enemy : enemyRobots) {
    ...
    public class RegressionAnalyzer {
        ...
        private double calculateModelError(RegressionModel model) {
            ...
            for (int i = 0; i < coordinates.size(); i++) {
                ...
                double error = getDistanceBetween(coordinate, predictedCoordinate);
                errorSum += error;
            }

            return errorSum / coordinates.size();
        }
    }
